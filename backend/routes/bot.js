...
// This file handles all incoming API calls from the Roblox bot.

const express = require('express');
const router = express.Router();
const db = require('../database/database');
const { body, validationResult } = require('express-validator');
const { authenticateBot } = require('../middleware/auth'); // Using the standard bot authentication

/**
 * @route   POST /api/bot/log-events
 * @desc    Endpoint for the bot to post duel log events.
    ...
    async (req, res) => {
        ...
        try {
            for (const event of events) {
                ...
                // Using the database pool directly handles transactions
                ...
                try {
                    ...
                    if (event.eventType === 'DUEL_STARTED' && websiteDuelIdFromEvent) {
                        const duelRes = await client.query('SELECT * FROM duels WHERE id = $1', [websiteDuelIdFromEvent]);
                        duel = duelRes.rows[0];
                        if (duel) {
                        ...
                        const duelRes = await client.query('SELECT * FROM duels WHERE bot_duel_id = $1', [event.duelId]);
                        duel = duelRes.rows[0];
                    }
                    
                    if (duel) {
                        let transcript = duel.transcript || []; // No need to parse if it's already a JSONB array
                        ...
                        if (event.eventType === 'PARSED_DUEL_ENDED') {
                            ...
                            if (winner_username) {
                                const winnerUserRes = await client.query('SELECT id FROM users WHERE linked_roblox_username = $1', [winner_username]);
